...
async function runTests() {
  ...
  try {
    ...
    customerA = await User.findOne({ email: 'customer_det_a@example.com' });
    if (!customerA) {
    ...
    customerB = await User.findOne({ email: 'customer_det_b@example.com' });
    if (!customerB) {
    ...
    agent = await User.findOne({ email: 'agent_det@example.com' });
    if (!agent) {
    ...
    admin = await User.findOne({ email: 'admin_det@example.com' });
    if (!admin) {
    ...
    try {
      ...
      console.error('Cleanup error in details tests:', cleanErr);
    }
